...
pub fn explore(year:&str, acs_est:&str) -> Result<HashMap<String, Vec<Table>>> {
...
fn format_table(year:&str, table: HashMap<String, Vec<Vec<String>>>, acs_est:&str ) -> HashMap<String, Vec<Table>> {
    ...
    for (table_id, table_details) in table.iter() {
        ...
        tables.entry(table_id.to_string()).or_insert(Vec::new()).push(t);
    }
